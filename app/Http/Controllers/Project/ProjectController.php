    ...
    public function show($projectId){
        ...
        return view('project_page.main',[
            ...
            "phase_participants" =>phase_participants::where('project_id',$projectId)->selectRaw("*
                , CASE
                    WHEN TIMESTAMPDIFF(MINUTE, NOW(), `time_to`) >= 0
                        AND `status` = 1
                        THEN 'subtask-color-1'
                    WHEN (TIMESTAMPDIFF(MINUTE, NOW(), `time_frome`) < 120 AND TIMESTAMPDIFF(MINUTE, NOW(), `time_frome`) >= 0 AND `status` = 0)
                        OR (TIMESTAMPDIFF(MINUTE, NOW(), `time_to`) < 120 AND TIMESTAMPDIFF(MINUTE, NOW(), `time_to`) >= 0 AND `status` = 1)
                        THEN 'subtask-color-2'
                    WHEN `status` = 2
                        THEN 'subtask-color-3'
                    WHEN (TIMESTAMPDIFF(MINUTE, NOW(), `time_frome`) < 0 AND TIMESTAMPDIFF(MINUTE, NOW(), `time_frome`) >= -1440 AND `status` = 0)
                        OR (TIMESTAMPDIFF(MINUTE, NOW(), `time_to`) < 0 AND TIMESTAMPDIFF(MINUTE, NOW(), `time_to`) >= -1440 AND `status` = 1)
                        THEN 'subtask-color-4'
                    WHEN (TIMESTAMPDIFF(MINUTE, NOW(), `time_frome`) <= -1440 AND `status` = 0)
                        OR (TIMESTAMPDIFF(MINUTE, NOW(), `time_to`) <= -1440 AND `status` = 1)
